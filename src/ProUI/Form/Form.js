...
export function ProInput(props) {
  const { type, placeholder, name, isRequired, errorMessage, label, labelPlacement, variant, color, onChange, ...rest } = props;
...
export function ProUploader(props) {
  const { placeholder, savePreviews = () => { }, multiple, action, onUpload = () => { }, ...rest } = props;
  const previewFile = (file, callback) => {
    const reader = new FileReader();
    reader.onloadend = () => callback(reader.result);
    reader.readAsDataURL(file);
