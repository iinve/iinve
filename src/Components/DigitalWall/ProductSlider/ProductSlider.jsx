...
const ProductCard = ({ data, isLightTheme = false, color }) => {
  ...
  return (
    <div
      className="relative bg-white/50 backdrop-blur-lg border rounded-2xl p-4 w-full h-fit flex flex-col justify-even shadow-lg transition-transform duration-300 ease-in-out cursor-pointer"
      style={{ borderColor: color?.highlight_color }}
    >
      <div className="flex justify-end items-center absolute -top-2 -right-2 !z-10">
        ...
        >
          <span className="text-white font-bold text-[12px] md:text-base w-fit flex items-center">
            <ProIcon
              name="FaRegGrinStars"
              size={20}
              color={color?.content_color}
            />{" "}
            <small className="text-base ml-2">New</small>
          </span>
        </div>
      </div>
      ...
      <div className={isLightTheme ? "text-black" : "text-white"}>
        <h3
          ...
          style={{ color: color?.content_color }}
        >
          {data?.title}
        </h3>
        <span
          className="text-sm block w-full opacity-70"
          style={{ color: color?.content_color }}
        >
          {data?.weight}
        </span>
