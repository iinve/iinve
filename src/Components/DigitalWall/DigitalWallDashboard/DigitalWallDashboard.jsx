...
export default function DigitalWallDashboard() {
  ...
  const [products, setProducts] = useState([
    { category: 0, title: "", weight: "", image: null, imagePreview: "" },
  ]);
  const [banners, setBanners] = useState([
    { image: null, imagePreview: "", text: "" },
  ]);
  const [newArrivals, setNewArrivals] = useState([
    { title: "", weight: "", image: null, imagePreview: "" },
  ]);
  ...
  const { colorFromImage, setColorFromImage } = useWallDashboard(
    companyDetails?.logo
  );
      ...
      try {
        ...
        setColorFromImage((prev) => {
          ...
          const filteredNewColors = newColors.filter(
            (newColor) =>
              newColor?.hex &&
              !safePrev.some((existing) => existing?.hex === newColor?.hex)
          );
  ...
  const handleImageUpload = async (e, section, index = null) => {
    const rawFile = e.target.files[0];
    const file =
      rawFile.size > 2 * 1024 * 1024 ? await compressImage(rawFile) : rawFile;
    if (!file) return;
  ...
  const removeImage = (section, index = null) => {
    if (section === "spotlight") {
      ...
      if (refs.current.newArrivals[index])
        refs.current.newArrivals[index].value = "";
    } else if (section === "logo") {
  ...
  return (
    <div className="bg-white min-h-screen text-gray-900">
      <div className="bg-blue-100 p-4 rounded-b-3xl relative z-10 lg:container mx-auto lg:px-24">
        <div className="flex justify-between items-center mb-10">
          ...
          <ActionButton
            ...
            size="md"
          >
            <ProIcon name="IoMdLogOut" size={18} color="white" /> Logout
          </ActionButton>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 mt-6 mb-4 px-4">
          <div className="flex items-center gap-4">
            <ProAvatar
              color="primary"
              url={companyDetails?.logo || ""}
              className="object-contain"
              size="md"
            />
            <h2 className="text-xl text-left block font-gray-200 text-blue-900 font-italic">
              {getGreeting()},<br />{" "}
              <span className="font-bold text-blue-900">{user?.shop_name}</span>
            </h2>
          </div>
        </div>
      </div>
      ...
      {isPageLoading ? (
        <div className="flex items-center justify-center h-screen fixed inset-0 w-full">
          <Spinner />
        </div>
      ) : (
        <div className="p-6 md:p-10 w-full md:w-2/3 xl:w-1/2 mx-auto !text-black">
          <Accordion variant="splitted" className="mb-6">
            ...
            <AccordionItem
              ...
              title={
                <div className="text-black flex items-center">
                  <ProIcon
                    name={"RiColorFilterAiLine"}
                    size={24}
                    color="#485ddc"
                  />{" "}
                  <span className="font-semibold ml-2">Customization</span>
                </div>
              }
            >
              <ThemeSelector
                colors={colorFromImage}
            ...
            <AccordionItem
              ...
              title={
                <div className="text-black flex items-center">
                  <ProIcon name={"IoIosAt"} size={24} color="#485ddc" />{" "}
          ...
          <section className="mb-8">
              ...
              {spotlight.image ? (
                <div className="w-full mx-auto">
                  <ImagePreview
                    ...
                    onRemove={() => removeImage("spotlight")}
                  />
                </div>
              ) : (
                <FileUploader
                  onUpload={(e) => handleImageUpload(e, "spotlight")}
                  refs={refs}
            ...
            <Textarea
              ...
              onChange={(e) =>
                setSpotlight({ ...spotlight, text: e.target.value })
              }
            />
          </section>
          ...
          <section className="mb-6">
              ...
              <div key={idx} className="flex items-center gap-2">
                ...
                {idx > 0 && (
                  <ActionButton
                    ...
                    className=""
                    size="md"
                    onPress={() => handleRemoveField(idx, "category")}
                  >
            ...
            <Button
              color="primary"
              variant="faded"
              onPress={() => setCategories([...categories, { name: "" }])}
                ...
                <div key={idx} className="flex items-center gap-2 relative">
                    ...
                    <div className="mb-2">
                      {prod.image ? (
                        <div>
                          <ImagePreview
                            image={prod.image}
                            onRemove={() => removeImage("products", idx)}
                            height={80}
                          />
                        </div>
                      ) : (
                        <FileUploader
                          onUpload={(e) =>
                            handleImageUpload(e, "products", idx)
                          }
                          refs={refs}
                          placeholder="Select Image"
                        />
                  ...
                  {idx > 0 && (
                    <ActionButton
                      ...
                      size="md"
                      onPress={() => handleRemoveField(idx, "product")}
                    >
                      <ProIcon name="CiTrash" size={18} color="white" />
                    </ActionButton>
                  )}
                ...
                <div key={idx} className="flex items-center gap-2 relative">
                  <div className="mb-4 p-2 border rounded w-full ">
                    <div className="mb-2">
                      {ban.image ? (
                        <ImagePreview
                          image={ban.image}
                          onRemove={() => removeImage("banners", idx)}
                        />
                      ) : (
                        <FileUploader
                          onUpload={(e) => handleImageUpload(e, "banners", idx)}
                          refs={refs}
                          placeholder="Select Image"
                        />
                      )}
                  ...
                  {idx > 0 && (
                    <ActionButton
                      ...
                      className="absolute -top-[20px] -right-[20px] z-10"
                      size="md"
                      onPress={() => handleRemoveField(idx, "banner")}
                    >
                      <ProIcon name="CiTrash" size={18} color="white" />
                    </ActionButton>
                  )}
                ...
                <div key={idx} className="flex items-center gap-2 relative">
                    ...
                    <div className="mb-2">
                      {item.image ? (
                        <div>
                          <ImagePreview
                            image={item.image}
                            onRemove={() => removeImage("newArrivals", idx)}
                          />
                        </div>
                      ) : (
                        <FileUploader
                          onUpload={(e) =>
                            handleImageUpload(e, "newArrivals", idx)
                          }
                          refs={refs}
                          placeholder="Select Image"
                        />
                  ...
                  {idx > 0 && (
                    <ActionButton
                      ...
                      size="md"
                      onPress={() => handleRemoveField(idx, "newArrival")}
                    >
                      <ProIcon name="CiTrash" size={18} color="white" />
                    </ActionButton>
                  )}
                ...
                <div key={index} className="relative">
                  ...
                  {index > 0 && (
                    <ActionButton
                      ...
                      size="sm"
                      onPress={() => handleRemoveField(index, "offer")}
                    >
                      <ProIcon name="CiTrash" size={18} color="white" />
                    </ActionButton>
                  )}
      ...
      <SuccessSheet
        isSuccess={isWallUpdated}
        user={user}
        closeSheet={() => setIsWallUpdated(false)}
      />
    </div>
  );
}
