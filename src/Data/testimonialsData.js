import { ASSETS } from "@/lib/assets";

export const testimonialsData =[
  {
    name:'Hadiq Thaha',
    designation:'Videographer',
    avatar: 'https://res.cloudinary.com/viiewme/image/upload/v1741446079/image_mmmmqm.jpg',
    message:"<span style='color: #485ddc;'>viiew.me</span> has completely transformed the way I showcase my work online! The sleek templates, effortless customization, and seamless experience make it the <span style='color: #485ddc;'>ultimate portfolio creator</span>. Highly recommend for creatives and professionals looking to stand out!",
    rating: 5,
    instagram:"https://www.instagram.com/had1q_"
  },
   {
    name:'Saheer Khan',
    designation:'Product Engineer at Reap',
    avatar: null,
    message:"I’ve been exploring <span style='color: #485ddc;'>viiew.me</span>, and it’s hands-down one of the easiest ways to create a sleek, professional portfolio. The platform offers beautifully designed templates, <span style='color: #485ddc;'>real-time editing</span>, and a <span style='color: #485ddc;'>seamless user experience</span>. Plus, the ability to customize and showcase work effortlessly makes it perfect for creators, developers, and freelancers.",
    rating: 5,
    linkedin:'https://www.linkedin.com/in/muhammedsaheerkhan/'
  },
  //  {
  //   name:'Afnaz Finu',
  //   designation:'Photographer | KBFC',
  //   avatar: ASSETS.testimonials.afnaz,
  //   message:"<span style='color: #485ddc;'>viiew.me</span> has completely transformed how I showcase my photography! As a KBFC photographer, I capture intense moments on the field, and with <span style='color: #485ddc;'>viiew.me</span>, I can present my best shots in a sleek and professional way. It's easy to update after every match, and sharing my work with clubs and clients has never been smoother!",
  //   rating: 5,
  //   instagram:'https://www.instagram.com/afnas_finu_'
  // },
   {
    name:'Samrajj',
    designation:'Software Engineer',
    avatar: null,
    message:"As a software engineer, having a professional online presence is crucial. <span style='color: #485ddc;'>viiew.me</span> made it incredibly easy to build a sleek portfolio without any hassle. <span style='color: #485ddc;'>I can showcase my projects, skills, and experience in one place</span>, and updating it takes seconds. ",
    rating: 5,
    linkedin:'https://www.linkedin.com/in/samrajj-keezhoth/'
  },
   {
    name:'Carlos Finley',
    designation:'Marketing Consultant',
    avatar: ASSETS.testimonials.carlos,
    message:"This is a powerful tool for designers, developers, and creatives alike. Providing a dedicated space for people to showcase their work can truly be a <span style='color: #485ddc;'>game changer</span> for landing new opportunities.",
    rating: 5,
  },
   {
    name:'Tyler Byron',
    designation:'IT Professional',
    avatar: ASSETS.testimonials.tyler,
    message:"I love that this platform combines customization with simplicity it’s perfect for creators who want a stunning portfolio <span style='color: #485ddc;'>without spending hours coding</span> ",
    rating: 5,
  },
   {
    name:'Luke Dunsmore',
    designation:'Founder of How About This',
    avatar: ASSETS.testimonials.luke,
    message:"Really like this its sleek, elegant and not overly complicated. Great work!",
    rating: 5,
  },
   {
    name:'Finn Archer',
    designation:'Marketing Manager',
    avatar: ASSETS.testimonials.finn,
    message:"This is absolutely brilliant.",
    rating: 5,
  },
  
]